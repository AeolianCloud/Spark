...
// opCols 是 vm_operations 的读取列清单。user_id 可为 NULL（预留列），
// error_message 有默认值，两者都经 COALESCE 归一化为空字符串/空指针。
const opCols = "id, node_id, pve_vmid, action, result, COALESCE(error_message, '') AS error_message, user_id, created_at"
...
func (r *VMOperationRepository) CreateOperation(ctx context.Context, op model.VMOperation) (*model.VMOperation, error) {
	...
	err := r.pool.QueryRow(ctx,
		"INSERT INTO vm_operations (node_id, pve_vmid, action, result, error_message, user_id) "+
		...
		&created.ErrorMessage, &created.UserID, &created.CreatedAt)
	if err != nil {
...
func (r *VMOperationRepository) ListOperations(ctx context.Context, nodeID, vmid int64, limit, offset int) ([]model.VMOperation, int, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&op.ID, &op.NodeID, &op.PVEVmid, &op.Action, &op.Result,
			&op.ErrorMessage, &op.UserID, &op.CreatedAt); err != nil {
			return nil, 0, classifyDBError(err)
		}
		out = append(out, op)
	}
